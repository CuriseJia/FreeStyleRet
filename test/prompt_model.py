...
class Prompt_ImageBind(nn.Module):
    def __init__(self, model_args):
        ...
        self.imagebind = imagebind_model.imagebind_huge(pretrained=self.args.imagebind_path)
    ...
    def forward(self, data, mode='image'):
        if mode == 'image':
            data = data + self.prompt.expand(data.shape[0], -1, -1).view(
                    data.shape[0],data.shape[1],data.shape[2],data.shape[3])
            input = {ModalityType.VISION: data.load_and_transform_vision_data(data, self.args.device)}
...
class Prompt_BLIP(nn.Module):
    def __init__(self, model_args):
        ...
        self.args = model_args
        self.blip = blip_retrieval(pretrained=self.args.blip_path, image_size=224, vit='large', vit_grad_ckpt=True, vit_ckpt_layer=10)
    ...
    def forward(self, data, mode='image'):
        ...
        if mode == 'image':
            ...
            ori_embed = self.blip.model.vision_proj(ori_feat)
            ...
            text_embed = self.blip.text_proj(text_output.last_hidden_state)
